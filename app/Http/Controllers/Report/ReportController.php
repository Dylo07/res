...
class ReportController extends Controller
{
    ...
    public function index(){
        // Show today's report automatically
        $today = date('m/d/Y');
        
        $dateStart = date("Y-m-d H:i:s", strtotime($today.' 00:00:00'));
        $dateEnd = date("Y-m-d H:i:s", strtotime($today.' 23:59:59'));

        $sales = Sale::whereBetween('updated_at', [$dateStart, $dateEnd])->where('sale_status','paid');
      
        $summarySales = Sale::select(
            'sale_details.menu_id',
            'menu_name',
            'categories.name',
            DB::raw('AVG(sale_details.menu_price) as avg_price'),
            DB::raw('SUM(sale_details.quantity) as qty_sum'),
            DB::raw('SUM(sale_details.menu_price * sale_details.quantity) as total_price')
        )
        ...
        // Detect suspicious bills (price mismatch)
        $suspiciousBills = Sale::select(
            'sales.id as sale_id',
            'sales.updated_at',
            'sale_details.menu_id',
            'menu_name',
            'menus.price as current_price',
            'sale_details.menu_price as billed_price',
            'sale_details.quantity',
            DB::raw('(menus.price - sale_details.menu_price) as price_difference'),
            DB::raw('(menus.price - sale_details.menu_price) * sale_details.quantity as total_difference')
        )
        ->join('sale_details', 'sales.id', '=', 'sale_details.sale_id')
        ->join('menus', 'menus.id', '=', 'sale_details.menu_id')
        ->whereBetween('sales.updated_at', [$dateStart, $dateEnd])
        ->where('sales.sale_status', 'paid')
        ->whereRaw('menus.price != sale_details.menu_price')
        ->orderBy('sales.updated_at', 'desc')
        ->get();

        return view('report.showReport')->with('dateStart', date("m/d/Y H:i:s", strtotime($today.' 00:00:00')))
        ->with('dateEnd', date("m/d/Y H:i:s", strtotime($today.' 23:59:59')))
        ->with('totalSale', $sales->sum('change'))
        ->with('serviceCharge', $sales->sum('total_recieved'))
        ->with('summarySales', $summarySales)
        ->with('suspiciousBills', $suspiciousBills)
